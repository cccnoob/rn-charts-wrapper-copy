...
public abstract class ChartBaseManager<T extends Chart, U extends Entry> extends SimpleViewManager<T> {
     ...
     */
    @ReactProp(name = "marker")
    public void setMarkerRate(Chart chart, ReadableMap propMap) {
        if (!BridgeUtils.validate(propMap, ReadableType.Boolean, "enabled") || !propMap.getBoolean("enabled")) {
            chart.setMarker(null);
            return;
        }

        RNMarkerView marker = new RNMarkerView(chart.getContext(),false);
        marker.setChartView(chart);
...
//        }
     ...
     */
    @ReactProp(name = "marker")
    public void setMarkerTotalIncome(Chart chart, ReadableMap propMap) {
        if (!BridgeUtils.validate(propMap, ReadableType.Boolean, "enabled") || !propMap.getBoolean("enabled")) {
            chart.setMarker(null);
            return;
        }

        RNMarkerView marker = new RNMarkerView(chart.getContext(),true);
        marker.setChartView(chart);
